...
export async function fetchEntries(query, { locale = "en-US" } = {}) {
...
export async function getHomePage({ locale = "en-US" } = {}) {
  const res = await fetchEntries(HOME_PAGE_QUERY, { locale });
  return res.items?.[0] || null;
}

export async function getSiteSettings({ locale = "en-US" } = {}) {
  const res = await fetchEntries(SITE_SETTINGS_QUERY, { locale });
  return res.items?.[0] || null;
}

export async function getProjects({ locale = "en-US" } = {}) {
  const res = await fetchEntries(PROJECTS_QUERY, { locale });
  return res.items || [];
}

export async function getAboutPage({ locale = "en-US" } = {}) {
  const res = await fetchEntries(ABOUT_PAGE_QUERY, { locale });
  return res.items?.[0] || null;
}

export async function getContactPage({ locale = "en-US" } = {}) {
  const res = await fetchEntries(CONTACT_PAGE_QUERY, { locale });
  return res.items?.[0] || null;
}